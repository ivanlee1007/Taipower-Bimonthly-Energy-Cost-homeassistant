...
> `sensor` 建議填 `*_power_cost` 或 `*_rate_status`，不要用 `*_kwh_cost`。前兩者會帶 `config_entry_id`，card 才能正確鎖定目標 integration。
>
> 如果這組 integration 之後改了來源 sensor，相關 entity_id 可能會跟著變，原本寫死的 `sensor:` 也要一起改；這就是為什麼正式環境建議直接改用 `entry_id`。
>
> 若 `sensor:` 寫錯、找不到、或該 sensor 沒有 `config_entry_id`，卡片會顯示錯誤訊息；這種情況請直接改成 `entry_id`。
...
>
> 現場驗證（v1.5.31）：同一頁放多張 card，各自綁不同 `entry_id` 後，住宅 / 商業設定可獨立修改，不會串改到另一組 integration。
...
| 版本 | 日期 | 說明 |
|------|------|------|
| v1.5.31 | 2026-03-26 | 設定卡片新增多組 integration 綁定能力：支援 `entry_id`（首選）與 `sensor` 綁定，讓同一個 dashboard 可各自編輯不同 config entry；現場閉環驗證住宅 / 商業互不串改 |
